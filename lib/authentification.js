...
function auditMethodesUtilisees(session, params, opts) {
  ...
  // Indiquer la methode primaire utilise, considerer deja verifiee
...
async function upgradeProteger(socket, params) {
  ...
  if(nombreVerifiees >= 2) {
    ...
    const sessionAuthChallenge = socket[CONST_WEBAUTHN_CHALLENGE].challenge
    for(let methode in methodesUtilisees) {
      ...
      if( ! params.verifie ) {
        if(methode.startsWith('webauthn.')) {
          ...
          try {
            const {counter} = await verifierChallenge(sessionAuthChallenge, compteUsager, params.valeur)
...
async function verifierMotdepasse(comptesUsagersDao, compteUsager, motdepasse) {
  ...
  try {
    ...
    debug("Erreur verifierMotdepasse usager %s : %O", nomUsager, err)
    return {valide: false}
  }
}
...
async function verifierTotp(comptesUsagersDao, compteUsager, tokenTotp) {
  ...
  try {
    ...
    return { valide: valide && valide.delta === 0 }
  } catch(err) {
    debug("Erreur verifierTotp pour usager %s : %O", compteUsager.nomUsager, err)
    return { valide: false }
  }
}
