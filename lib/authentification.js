...
const { validerChaineCertificats, splitPEMCerts, extraireExtensionsMillegrille } = require('./forgecommon')
...
//   }
//
//   const infoUsager = await req.comptesUsagersDao.chargerCompte(nomUsager, fingerprintPk)
...
//     }
//
//     var challengeCertificat = req.session[CONST_CHALLENGE_CERTIFICAT]
...
//       }
//       req.session[CONST_CHALLENGE_CERTIFICAT] = challengeCertificat
//     }
  ...
  */
  ...
  if(compteUsager) {
    ...
    if(compteUsager.webauthn) {
      ...
      // Conserver challenge pour verif
      socket[CONST_WEBAUTHN_CHALLENGE] = challengeWebauthn.challenge

      reponse.challengeWebauthn = challengeWebauthn
    }

    if(socket[CONST_AUTH_PRIMAIRE]) {
      reponse[CONST_AUTH_PRIMAIRE] = socket[CONST_AUTH_PRIMAIRE]
    }
    if(socket[CONST_AUTH_SECONDAIRE]) {
      reponse[CONST_AUTH_SECONDAIRE] = socket[CONST_AUTH_SECONDAIRE]
    }
...
async function genererChallengeWebAuthn(socket, params) {
  params = params || {}
  const session = socket.handshake.session,
        nomUsager = socket.nomUsager || session.nomUsager,
        userId = session.userId
  debug("genererChallenge2FA: Preparation challenge usager : %s, params: %O\nsession: %O", nomUsager, params, session)
  ...
  const compteUsager = await comptesUsagersDao.chargerCompte(nomUsager)
  ...
  if(compteUsager) {
    ...
    var challengeCertificat = session[CONST_CHALLENGE_CERTIFICAT]
    if(!challengeCertificat) {
      // Generer nouveau challenge
      challengeCertificat = {
        date: new Date().getTime(),
        data: Buffer.from(randomBytes(32)).toString('base64'),
      }
      session[CONST_CHALLENGE_CERTIFICAT] = challengeCertificat
    }
    reponse.challengeCertificat = challengeCertificat
...
function auditMethodesUtilisees(session, params, opts) {
  ...
  if(params.cleMillegrille) {
    methodesUtilisees.cleMillegrille = {
      ...
      challengeSession,
      // challengeSocket: socket[CONST_CERTIFICAT_AUTH_CHALLENGE],
      verifie: false
    }
  }
...
async function upgradeProtegeCertificat(socket, params) {
  ...
  console.debug("authentification.upgradeProtegeCertificat Params recus : %O, challenge session %O", params, challengeSession)
...
async function upgradeProteger(socket, params) {
  ...
  if( params.nouvelEnregistrement && params.methodeVerifiee && methodesUtilisees.certificat && methodesUtilisees.certificat.verifie ) {
    // C'est un nouvel enregistrement (e.g. certificat + U2F). Valide.
    methodesUtilisees[params.methodeVerifiee] = {verifie: true}
    methodesReverifiees.push(params.methodeVerifiee)
    debug("authentification.upgradeProteger nouvel enregistrement\nutilisees : %O, methode enregistrement: %O", methodesUtilisees, params.methodeVerifiee)
    ...
    for(let methode in methodesUtilisees) {
      ...
      if( ! params.verifie ) {
        try {
          // Verifier la methode.
          // Output est dans les flags de params
          await verifierMethode(socket, methode, compteUsager, params)
        } catch(err) {
...
async function verifierSignatureCertificat(idmg, chainePem, challengeSession, challengeBody) {
  ...
  if(!idmg || idmg !== idmgChaine) {
    ...
    if(valide) {
      return { valide, certificat, idmg, nomUsager, userId }
    }
...
module.exports = {
  ...
  // verifierTotp,
  genererChallengeCertificat, verifierMethode,
