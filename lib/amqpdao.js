...
class MilleGrillesAmqpDAO {
  ...
  async transmettreTransactionFormattee(message, domaineAction, opts) {
    // if(!opts) opts = {};
    //
    ...
    // let promise = new Promise((resolve, reject)=>{resolve(messageFormatte)});
    //
    ...
    const messageFormatte = await this.pki.formatterMessage(message, domaineAction, opts)
    const routingKey = 'transaction.' + domaineAction
    const correlation = infoTransaction['uuid_transaction'];
    const messageString = JSON.stringify(messageATransmettre)
  ...
  //     transactionCle = await formatterMessage(transactionCle, routingKeyCle, opts)
  ...
  transmettreReponse(message, replyTo, correlationId) {
    const messageFormatte = this.pki.formatterMessage(message);
  ...
  async transmettreCommande(domaineAction, message, opts) {
    ...
    var infoTransaction = message['en-tete']
    if( ! opts.noformat || ! infoTransaction ) {
  ...
  async emettreEvenement(message, routingKey) {

    // const infoTransaction = this._formatterInfoTransaction(routingKey);
    // message['en-tete'] = infoTransaction;
    // this._signerMessage(message);
    message = await this.pki.formatterMessage(message, routingKey, opts)

    const jsonMessage = JSON.stringify(message);

    // Return promise
    return this._transmettre(routingKey, jsonMessage);
  }
