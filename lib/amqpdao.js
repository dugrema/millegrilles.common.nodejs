...
class MilleGrillesAmqpDAO {
  ...
  _connect() {
    ...
    if(this.connection === null) {
      ...
      .then( conn => {
        ...
        let fingerprint = this.certificatManager.transmettreCertificat()
    ...
    .catch(err=>{
      if(err.inconnu) {
        ...
        if(json_message.certificat_pem) {
          ...
          return this.certificatManager.demanderCertificat(fingerprint)
          .then(reponse=>{
  ...
  traiterMessageValide(json_message, msg, callback) {
    ...
    if(exchange === '' && callback) {  // Exclure message sur exchange, uniquement utiliser direct Q ('')
      debug("Reponse message direct, correction : %s", msg.properties.correlationId)
      callback(json_message)
    } else if(routingKey) {
      // Traiter le message via handlers
      debug("traiterMessageValide par routing keys:\nFields: %O\nProperties: %O\n%O", msg.fields, msg.properties, json_message)
