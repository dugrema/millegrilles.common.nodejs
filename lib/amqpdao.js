...
class MilleGrillesAmqpDAO {
  ...
  _connect() {
    ...
    if(this.connection === null) {
      ...
      .then( conn => {
        ...
        return this.channel.bindQueue(this.reply_q.queue, this.exchange, MG_ROUTING_EMETTRE_CERTIFICAT)
      }).catch(err => {
        this.connection = null;
        ...
        try {
          if(err.message.indexOf('403') > 0) {
            ...
            var host = this.url.split('/')[2]  // Extraire de amqps://HOST:port/vhost
            host = host.split(':')[0]  // Split host:port
            ...
            .catch(err=>{
              console.error("Erreur (1) tentative de creation de compte MQ\n%s", err)
              debug("Erreur (1-detail) tentative de creation de compte MQ\n%s", err)
            })
          } else {
            console.error("Erreur (2) connexion RabbitMQ : %s", err.message);
            debug("Erreur (2-detail) connexion RabbitMQ:\n%O", err)
          }
        } catch(err) {
          console.error("Erreur (3) tentative de creation de compte MQ\n%s", err.message)
  ...
  async _traiterMessage(msg) {
    ...
    let callbackInfo = this.pendingResponses[correlationId];
    let callback = null
    if(callbackInfo) {
    ...
    .then(signatureValide=>{
      if(signatureValide) {
        ...
        if(callbackInfo && !callbackInfo.nodelete) {
          delete this.pendingResponses[correlationId]
        }
      }
    })
    .catch(err=>{
      if(err.inconnu) {
        ...
        // certificat.
        if(json_message.certificat_pem) {
          // On laisse le message passer, c'est un certificat
          debug("Certificat recu: %O", json_message);
          if(callback) {
            callback(msg);
          } else {
            debug("Message certificat recu, aucun callback")
            this.pki.sauvegarderMessageCertificat(messageContent)
          }
        } else {
          ...
          .then(reponse=>{
            ...
            if(!etatCertificat) {
              ...
              this.pki.sauvegarderMessageCertificat(JSON.stringify(reponse))
  ...
  traiterMessageValide(json_message, msg, callback) {
    ...
    if(callback) {
      ...
      debug("traiterMessageValide:\n%O", msg)
      return this.routingKeyManager.handleMessage(routingKey, msg.content, {properties: msg.properties, fields: msg.fields});
    } else {
  ...
  transmettreEnveloppeTransaction(transactionFormattee, domaine) {
    ...
    const routingKey = 'transaction.' + domaine  // 'transaction.nouvelle';
  ...
  transmettreRequete(domaineAction, message, opts) {
    ...
    // debug(jsonMessage)
    var promise = this._transmettre(routingKey, jsonMessage, correlation, opts);
  ...
  transmettreRequeteMultiDomaines(domaineAction, message, opts) {
    ...
    const correlationId = infoTransaction['uuid-transaction'];
    const jsonMessage = JSON.stringify(message);
  ...
  _transmettre(routingKey, jsonMessage, correlationId, opts) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      if(correlationId) {
        ...
        pendingResponses[correlationId] = {callback: fonction_callback, creationDate: new Date()}
...
class RoutingKeyManager {
    ...
    if(callback) {
      ...
      try {
        ...
        if(TYPES_MESSAGES_ROOM_ACCEPTES.includes(splitKey[0]) && exchange !== '4.secure') {
          ...
          debug("Emission evenement sur rooms %s Socket.IO\n%O", rooms, contenuEvenement)
  ...
  addRoutingKeysForSocket(socket, routingKeys, niveauSecurite, channel, reply_q) {
    ...
    for(var routingKey_idx in routingKeys) {
      ...
      this.roomsEvenements[routingKeyName] = {exchange, reply_q, roomName}
    }
  }
  ...
  nettoyerRooms() {
    if(this.socketio) {
      ...
      for(let routingKeyName in this.roomsEvenements) {
        const roomConfig = this.roomsEvenements[routingKeyName]
        const socketRoom = rooms[roomConfig.roomName]
        if(!socketRoom) {
          ...
          debug("Routing key %s, room %s, %d membres", routingKeyName, roomConfig.roomName, socketRoom.length)
        }
      }

      // const rooms = this.socketio.sockets.rooms
      debug("SocketIO rooms: %s", rooms)
      debug("Entretien rooms, nb rooms : %d", Object.keys(rooms).length)
      for(let room in rooms) {
        const roomInfo = rooms[room]
        debug("Room %s, membres %d\n%O", room, roomInfo.length, roomInfo)
      }
    } else {
