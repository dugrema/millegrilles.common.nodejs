...
class MilleGrillesAmqpDAO {
  ...
  async _traiterMessage(msg) {
    ...
    if( routingKey && routingKey === MG_ROUTING_EMETTRE_CERTIFICAT ) {
      ...
      this.certificatManager.sauvegarderMessageCertificat(messageContent, fingerprint)
    ...
    if(err.inconnu) {
      ...
      if(messageDict.chaine_pem) {
        ...
        if(callback) {
          ...
          this.certificatManager.sauvegarderMessageCertificat(messageContent, messageDict.fingerprint_sha256_b64)
        }
      } else {
