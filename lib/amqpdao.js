  ...
  _connect() {
    ...
    if(this.connection === null) {
      ...
      .then( conn => {
        ...
        this.channel.bindQueue(this.reply_q.queue, MG_EXCHANGE_PROTEGE, this.routingKeyCertificat);
        this.channel.bindQueue(this.reply_q.queue, MG_EXCHANGE_PROTEGE, MG_ROUTING_EMETTRE_CERTIFICAT);
      }).catch(err => {
        this.connection = null;
...
class RoutingKeyManager {
  ...
  addRoutingKeyCallback(callback, routingKeys, opts) {
    ...
    for(var routingKey_idx in routingKeys) {
      ...
      if(operationLongue) {
        ...
        this.mq.channel.bindQueue(this.mq.qOperationLongue.queue, MG_EXCHANGE_PROTEGE, routingKeyName);
      } else {
        debug("Ajouter callback pour routingKey %s", routingKeyName);
        this.mq.channel.bindQueue(this.mq.reply_q.queue, MG_EXCHANGE_PROTEGE, routingKeyName);
      }
    }
  }
  ...
  removeRoutingKeys(routingKeys) {
    for(var routingKey_idx in routingKeys) {
      ...
      this.mq.channel.unbindQueue(this.mq.reply_q.queue, MG_EXCHANGE_PROTEGE, routingKeyName);
    }
  }
  ...
  addRoutingKeysForSocket(socket, routingKeys, channel, reply_q) {
    ...
    for(var routingKey_idx in routingKeys) {
      ...
      // Ajouter la routing key
      this.mq.channel.bindQueue(reply_q.queue, MG_EXCHANGE_PROTEGE, routingKeyName);
    }
  }
  ...
  removeRoutingKeysForSocket(socket, routingKeys, channel, reply_q) {
    for(var routingKey_idx in routingKeys) {
      ...
      // Retirer la routing key
      this.mq.channel.unbindQueue(reply_q.queue, MG_EXCHANGE_PROTEGE, routingKeyName);
    }
  }
