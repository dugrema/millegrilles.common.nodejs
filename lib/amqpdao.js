...
class MilleGrillesAmqpDAO {
  ...
  async _traiterMessage(msg) {
    ...
    if(routingKey && routingKey.startsWith(MG_ROUTING_EMETTRE_CERTIFICAT)) {
      ...
      this.certificatManager.sauvegarderMessageCertificat(messageContent, messageDict.fingerprint)
    ...
    if(err.inconnu) {
      ...
      if(messageDict.certificat_pem) {
        ...
        if(callback) {
          ...
          this.certificatManager.sauvegarderMessageCertificat(messageContent)
        }
      } else {
        // On tente de charger le certificat
        let fingerprint = messageDict['en-tete'].certificat;
        debug("Certificat inconnu, on fait une demande : " + fingerprint);

        const reponse = await this.certificatManager.demanderCertificat(fingerprint)
        debug("Reception certificat apres demande :\n%O", reponse)
    //     .then(reponse=>{
  ...
  async transmettreRequete(domaineAction, message, opts) {
    ...
    if( ! opts.nowait ) {
      correlationId = infoTransaction['uuid-transaction']
    }
  ...
  _transmettre(routingKey, jsonMessage, correlationId, opts) {
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      this.channel.publish(
        ...
        function(err, ok) {
          if(err) {
            debug("Erreur MQ Callback : %O", err);
            delete this.pendingResponses[correlationId];
            reject(err);
          }
        }
