...
function _initSocketIo(server, paramsSocketio, opts) {
  ...
  socketIo.on('connection', async (socket) => {
    ...
    try {
      ...
      socket.listenersProprietaires = []
      socket.upgradeProtege = _=>{upgradeProtege(socket)}
      socket.upgradeProprietaire = _=>{upgradeProprietaire(socket)}
      socket.downgradePrive = downgradePrive
      ...
      if(socket.modeProtege) {
        if(socket.estProprietaire) {
          socket.upgradeProprietaire()
        } else {
...
function upgradeProtege(socket) {
  debug("Mode protege - usager, %d hooks", socket.listenersProteges.length)

  // Enregistrer tous les listeners proteges
  if(socket.listenersProteges) enregistrerListener(socket.listenersProteges)
}

function upgradeProprietaire(socket) {
  debug(
    "Mode protege - proprietaire, %d hooks proteges, %d hooks proprietaires",
    socket.listenersProteges.length,
    socket.listenersProprietaires.length,
  )

  // Enregistrer tous les listeners proteges
  if(socket.listenersProteges) enregistrerListener(socket, socket.listenersProteges)

  // Enregistrer tous les listeners proprietaires
  if(socket.listenersProprietaires) enregistrerListener(socket, socket.listenersProprietaires)
}
...
function downgradePrive(socket) {
  // Retirer tous les listeners proteges et proprietaires
  if(socket.listenersProteges) {
    socket.listenersProteges.forEach(listener => {
      debug("Retrait listener %s", listener.eventName)
      socket.removeAllListeners(listener.eventName)
    })
  }

  if(socket.listenersProprietaires) {
    socket.listenersProprietaires.forEach(listener => {
      debug("Retrait listener %s", listener.eventName)
      socket.removeAllListeners(listener.eventName)
    })
  }
