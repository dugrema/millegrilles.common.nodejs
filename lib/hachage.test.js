const {hacher, verifierHachage, Hacheur, VerificateurHachage, calculerPassword} = require('./hachage')
...
describe.only('pbkdf2', ()=>{
  ...
  test('password test', async ()=>{
    const key = await calculerPassword('test', 121259, 'mFiPRhe+eNT4qivXCgJ6cUg')
  ...
  test('password chars utf8', async ()=>{
    ...
    const key = await calculerPassword(password, 84447, 'mDcz2/foFgqC54IGHLzPLig')
