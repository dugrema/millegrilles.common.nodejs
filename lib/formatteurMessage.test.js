const {hacherMessage, SignateurMessage, FormatteurMessage, hacherCertificat} = require('./formatteurMessage')
...
const FINGERPRINT_CERTIFIAT = 'zQmT8L1ctRhSV6QAVXWm7bGs9uR14K9whuUnAKanC4Pz84T'
const PEM_CERT = `
-----BEGIN CERTIFICATE-----
...
describe('hacher message', ()=>{
  ...
  test('hacher certificat 1', async ()=>{
    const cert = forgePki.certificateFromPem(PEM_CERT)
    const hachage = await hacherCertificat(cert)
    console.debug("Certificat 1 hachage : %s ", hachage)
    expect(hachage).toBe(FINGERPRINT_CERTIFIAT)
  })
...
describe('formatter message', ()=>{
  ...
  test('formatter message 1', async ()=>{
    ...
    expect(resultat['en-tete'].fingerprint_certificat).toBe(FINGERPRINT_CERTIFIAT)
  ...
  test('formatter message 2 avec certificats', async ()=>{
    const resultat = await formatteur.formatterMessage(MESSAGE_2, 'Domaine.test', {attacherCertificat: true})
    console.debug("Resultat formatter message 1\n%O", resultat)
    expect(resultat['_certificat']).not.toBeNull()
  })
