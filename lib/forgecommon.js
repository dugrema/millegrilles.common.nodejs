...
const {hacherCertificat} = require('./formatteurMessage')
...
async function validerChaineCertificats(chainePEM, opts) {
  ...
  var idmg = opts.idmg
  if(idmg && organization !== idmg) {
    throw new Error("Certificat (O=" + organization + ") ne corespond pas au IDMG " + opts.idmg)
  } else {
    verifierIdmg(organization, certCa)
...
//   }
//
//   async verifierCertificat(chainePem) {
//     return validerChaineCertificats(chainePem, {
...
// function hacherDictionnaire(transaction, opts) {
//   opts = opts || {}
//   const fonctionHachage = opts.fonction || 'sha256'
//   let hachage_transaction = 'N/A';
//   const hash = crypto.createHash('sha256');
//
//   // Copier transaction sans l'entete
...
//   hash.write(transactionJson, 'utf-8');
//
//   hachage_transaction = fonctionHachage + '_b64:' + hash.digest('base64')
//
//   return hachage_transaction;
// }

// function calculerHachageBytes(contenu, opts) {
//   if(!opts) opts = {}
//
//   var encoding = opts.encoding || 'base64'
//
//   const hachage = opts.hash || 'sha512'
//   console.debug("Calcul digest %s sur %O: ", hachage, contenu)
//
//   var bytes = contenu
//   if(encoding === 'base64') {
//     bytes = util.decode64(bytes)
//   }
//
//   const signMd = md[hachage].create()
//   signMd.update(bytes)
//   const digestBytes = signMd.digest().getBytes()
//   const resultat = hachage + '_b64:' + util.encode64(digestBytes)
//
//   debug("Digest : %s", resultat)
//
//   return resultat
// }
...
module.exports = {
  ...
  chargerClePrivee, chiffrerPrivateKey, calculerHachageCertificatPEM,
  ...
  // extraireClePubliquePEM, extraireInformationCertificat,
  // chargerClePubliquePEM,
  // signerContenuString, verifierSignatureString,
  // chargerCertificatPEM,
  // hacherDictionnaire, calculerHachageBytes,
