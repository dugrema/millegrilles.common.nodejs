...
function verifierSignatureString(clePublique, contenuString, signature, opts) {
  ...
  try {
    ...
    const saltLength = (modulusLength - 512) / 8 - 2
...
function extraireInformationCertificat(pem) {
  ...
  var fingerprintBase64 = md.sha1.create()
    .update(asn1.toDer(pki.certificateToAsn1(cert)).getBytes())
    .digest()
  fingerprintBase64 = util.encode64(fingerprintBase64.getBytes())

  return({cert, fingerprint, clePubliquePem: clePublique, fingerprintBase64})
}
...
class SignateurTransaction {
  ...
  constructor(chainePem, cle) {
    ...
    this.fingerprint = infoCert.fingerprint
  }
  ...
  async preparerTransaction(transaction, domaine, opts) {
    ...
    transaction['en-tete']['hachage-contenu'] = hachage
