...
const {extraireInformationCertificat, SignateurTransaction, comparerArraybuffers} = require('./forgecommon')
...
export class CryptageAsymetrique {
  ...
  crypterCleSecreteAb(clePublique, cleSecreteBuffer) {
    /**
    Chiffre une cle secrete, cleSecrete est un ArrayBuffer.
    **/
    let clePubliqueBuffer = str2ab(window.atob(clePublique));
...
export class CryptageSymetrique {
  ...
  crypterContenu(buffer) {
    ...
    .then(clesIv=>{
      ...
      var prepBuffer = this.preparerBufferChiffrage(buffer, clesIv.iv)
      // new Uint8Array(clesIv.iv.byteLength + buffer.byteLength);
    ...
    .then(bufferCrypte=>{
      //console.debug("Fichier crypte dans buffer : %O", bufferCrypte);
      return ({...clesIvLocal, bufferCrypte});
    });
  }

  preparerBufferChiffrage(buffer, iv) {
...
export class CryptageSymmetriqueGCM extends CryptageSymetrique {
  getParamsAlgorithme() {
    // Chiffrage avec GCM
  ...
  preparerBufferChiffrage(buffer) {
    // Aucune preparation speciale requise pour GCM
    return buffer
  }

  crypterContenu(buffer) {
    // Executer le chiffrage puis extraire le computeTag GCM
    return super.crypterContenu(buffer)
      .then(resultat => {
        // Extraire le compute tag GCM
        var bufferCrypte = resultat.bufferCrypte
        const viewCrypte = new Uint8Array(bufferCrypte)
        const longueurBuffer = viewCrypte.length
        const computeTag = viewCrypte.slice(longueurBuffer-16)
        bufferCrypte = viewCrypte.slice(0, longueurBuffer-16)

        return {
          ...resultat, bufferCrypte, computeTag
        }
      })
  }

  decrypterContenu(buffer, cleSecrete, iv, computeTagBase64) {
    // Concatener le buffer et le compute tag - c'est le format requis par subtle
    var computeTagTemp = atob(computeTagBase64)
    const computeTag = new Uint8Array(computeTagTemp.length);
    for(let i=0; i<computeTagTemp.length; i++) {
      computeTag[i] = computeTagTemp.charCodeAt(i);
    }

    console.debug("Compute tag : %O", computeTag)
    const concatBuffer = new Uint8Array(computeTag.length + buffer.byteLength)
    console.debug("Concat buffer : %d", concatBuffer.length)
    concatBuffer.set(new Uint8Array(buffer), 0);
    concatBuffer.set(new Uint8Array(computeTag), buffer.byteLength);
    return super.decrypterContenu(concatBuffer, cleSecrete, iv)
  }
}
...
export class MilleGrillesCryptoHelper {
  ...
  getParamsAlgorithme() {
    return {
      algorithm: 'aes-256-cbc',  // Meme algorithme utilise sur MG en Python (mgs1)
  ...
  async chiffrerDictionnaire(signateurTransaction, certificatMaitredescles, identificateurs_document, domaine, contenu) {
    ...
    const hachage_bytes = await this.calculerHachageArrayBuffer(contenuChiffre.bufferCrypte)
    ...
    await signateurTransaction.preparerTransaction(transactionMaitredescles, 'MaitreDesCles.sauvegarderCle')
  ...
  crypter(dictACrypter, clePublique) {
    ...
    return new Promise((resolve, reject)=>{
      ...
      .then(result=>{
        ...
        return cryptageAsymetrique.crypterCleSecreteAb(clePublique, cleSecrete);
      })
      ...
      reader.onload = async _ => {
        try {
          ...
          for(let idx in pemCerts) {
            ...
            const {fingerprintSha256Base64, clePubliquePem: clePublique} = extraireInformationCertificat(pem)
            ...
            clesChiffreesParCert[fingerprintSha256Base64] = btoa(String.fromCharCode.apply(null, new Uint8Array(cleSecreteChiffree)))
          }
  ...
  creerCipherKey() {
    let promise = new Promise((resolve, reject) => {
      this.genererSecret((err, {key, iv})=>{
        ...
        var cipher = crypto.createCipheriv(this.getParamsAlgorithme().algorithm, key, iv);
  ...
  decrypter(contenuCrypte, cleSecrete, iv) {
    return new Promise((resolve, reject)=>{
      ...
      var decipher = crypto.createDecipheriv(this.getParamsAlgorithme().algorithm, cleSecreteBuffer, ivBuffer);
...
export async function calculerHachageArrayBuffer(arrayBuffer, opts) {
  if(!opts) opts = {}
  const hachage = opts.hash || 'sha2-512'

  console.debug("Calcul digest %s sur %O: ", hachage, arrayBuffer)

  var hachageSubtle = hachage
