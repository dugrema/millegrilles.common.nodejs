...
export class CryptageAsymetrique {
  ...
  async genererKeysNavigateur(opts) {
    ...
    if(window && window.crypto && window.crypto.subtle) {
      ...
      const clePriveePEM = await window.crypto.subtle.exportKey("pkcs8", keypair.privateKey)

      try {
        const clePriveeDecrypt = await window.crypto.subtle.importKey(
          "pkcs8", clePriveePEM, {name: this.algorithm, hash: this.hashFunction}, false, ['decrypt']
        )
        ...
        const clePriveeSigner = await window.crypto.subtle.importKey(
          'pkcs8', clePriveePEM, {name: 'RSA-PSS', hash: 'SHA-512'}, false, ['sign']
        )
        console.debug("Cle signature : %O", clePriveeSigner)

        return {
          clePublique: keypair.publicKey,
          clePriveeDecrypt, clePriveeSigner
        }
      } catch(err) {
