...
async function chiffrerSubtle(contenu, opts) {
  ...
  console.debug("_getRandomValues : %O", _getRandomValues)
...
async function chiffrerCleSecreteSubtle(clePublique, cleSecrete, opts) {
  ...
  var clePubliqueBuffer = null
  if(typeof(clePublique) ==='string') {
    clePubliqueBuffer = Buffer.from(clePublique, 'base64')
  } else if( clePublique.verify ) {
    if(DEBUG) console.debug("Cle publique format forge, on l'exporte en PEM")
    const clePubliquePem = forgePki.publicKeyToPem(clePublique)
    if(DEBUG) console.debug("Cle publique format PEM : %O", clePubliquePem)
    const regEx = /\n?\-{5}[A-Z ]+\-{5}\n?/g
    var clePubliqueString = clePubliquePem.replaceAll(regEx, '')
    if(DEBUG) console.debug("Cle publique string : %O", clePubliqueString)
    clePubliqueBuffer = Buffer.from(clePubliqueString, 'base64')
  } else {
    console.error("Format cle publique inconnu : %O", clePublique)
    throw new Error("Format cle publique inconnu")
  }
