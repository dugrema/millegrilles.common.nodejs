...
async function dechiffrerCleSecreteSubtle(clePrivee, cleSecreteChiffree, opts) {
  ...
  if( true ) { // -- CLE PRIVEE PAS SUBTLE --
    clePrivee = await _importerClePrivee(clePrivee, opts)
  }
...
function _importerClePrivee(clePrivee, opts) {
  opts = opts || {}
  const algorithm = opts.algorithm || 'RSA-OAEP',
  ...
  if(typeof(clePrivee) === 'string') {
    ...
    if(opts.DEBUG) console.debug("Cle privee stripee :\n%s", clePrivee)
