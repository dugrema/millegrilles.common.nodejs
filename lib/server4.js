...
const { genererChallengeCertificat } = require('./authentification')
...
function socketActionsMiddleware(configurerEvenements, opts) {
  ...
  const middleware = (socket, next) => {
    ...
    if(!session.userId) {
      session.nomUsager = headers['x-user-name']
      session.userId = headers['x-user-id']
      session.authScore = authScore
      session.save()
    }

    // Enregistrer evenements publics de l'application
    enregistrerListener(socket, configurationEvenements.listenersPublics)
    socket.activerListenersPrives = _ => enregistrerListenersPrives(socket, configurationEvenements.listenersPrives)
...
// async function verifierNavigateur(socket) {
//   /* Emet un challenge au navigateur sur reconnexion (session deja active)
//      pour reactiver les listeners proteges */
//   const challengeServeur = await genererChallengeCertificat(socket)
//   debug("server4.verifierNavigateur challenge : %O", challengeServeur)
//
//   socket.emit('challengeCertificat', challengeServeur, reponse=>{
//     debug("Reponse challenge certificat : %O", reponse)
//     const chainePem = reponse['_certificat']
//     const resultatVerification = verifierSignatureCertificat(idmg, chainePem, challengeServeur, reponse)
//     if(resultatVerification[1] === true) {
//       socket.activerModeProtege()
//       debug("Certificat OK, ajout listeners apres upgrade protege pour usager %s : %O", Object.keys(socket._events))
//     }
//   })
// }

function enregistrerListenersPrives(socket, listenersPrives) {
  const session = socket.handshake.session
  const {nomUsager} = session
...
function activerModeProtege(socket, listenersProteges) {
  const session = socket.handshake.session
  // const {nomUsager, auth} = session
  //
  // if(!auth || auth.length === 0) {
  //   // Refuser l'upgrade, la session n'est pas prete
  //   return false
  // }
