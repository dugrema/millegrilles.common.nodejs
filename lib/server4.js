...
async function initialiser(app, configurerEvenements, opts) {
  ...
  if(opts.exchange) {
    amqpdao.exchange = opts.exchange
  }
  ...
  if( ! opts.public ) app.use(verifierAuthentification)
...
function socketActionsMiddleware(configurerEvenements, opts) {
  ...
  const middleware = (socket, next) => {
    ...
    if(!opts.public) {
      ...
      enregistrerListener(socket, socket.configurationEvenements.listenersPublics)
    }
