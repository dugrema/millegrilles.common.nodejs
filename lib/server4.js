...
const { genererChallengeCertificat, upgradeProtegeCertificat } = require('./authentification')
...
function socketActionsMiddleware(configurerEvenements, opts) {
  ...
  const middleware = (socket, next) => {
    ...
    debug("Req headers : %O", socket.handshake.headers)
    if(!userId) {
      console.error("ERREUR socketActionsMiddleware : headers.user-id n'est pas fourni")
    ...
    // Enregistrer evenements prives
    enregistrerListener(socket, socket.configurationEvenements.listenersPrives)
...
function transferHeaders(req, res, next) {
  ...
  if( ! session.nomUsager ) {
    const nomUsager = req.headers.user
    if(!nomUsager) {
      debug("Nom usager n'est pas inclus dans les req.headers")
