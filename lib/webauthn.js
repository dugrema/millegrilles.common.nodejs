...
//
//     // Generer userId avec info usager, idmg
//     const idmg = req.amqpdao.pki.idmg
//     const valeurHachage = [nomUsager, idmg].join(':')
//     userId = await hacher(valeurHachage, {hashingCode: 'sha2-256', encoding: 'base58btc'})
//     debug("Valeur hachage : '%s'\nuserId: %s", valeurHachage, userId)
...
//     hostname: registrationChallenge.hostname,
//     nomUsager,
//     attestationExpectations: registrationChallenge.attestationExpectations,
//   }
//   req.session.save()
//   debug("Etat session : %O", req.session)
//
//   return res.send({
...
async function verifierChallenge(challengeInfo, compteUsager, clientAssertionResponse) {
  const authChallenge = challengeInfo.challenge,
        rpId = challengeInfo.rpId
  debug("authentifier: challenge : %O\ncompteUsager: %O\nauthResponse: %O", challengeInfo, compteUsager, clientAssertionResponse)
  ...
  return {authentifie: true, counter, userVerification, userPresence}
}
...
module.exports = {
  ...
  // authentifier, verifierChallengeRegistration, genererChallengeRegistration,
}
