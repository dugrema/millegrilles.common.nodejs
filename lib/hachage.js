...
const {pbkdf2: cryptoPbkdf2} = require('crypto')
const {util: forgeUtil, md: forgeMd, asn1: forgeAsn1, pki: forgePki, pkcs5: forgePkcs5} = require('node-forge')
...
function hacherPassword(password, iterations, salt) {
  // Retourne promise
  if(_subtle) {
...
async function hacherPasswordCrypto(password, iterations, salt) {
  ...
  var key = await new Promise((resolve, reject)=>{
    cryptoPbkdf2(password, saltBuffer, iterations, 32, 'sha256', (err, derivedKey)=>{
...
async function hacherPasswordSubtle(password, iterations, salt) {
  var saltBuffer = multibase.decode(salt)
...
module.exports = {
  ...
  hacherPassword, hacherPasswordCrypto, hacherPasswordSubtle,
}
