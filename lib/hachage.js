...
class Hacheur {
  ...
  finalize() {
    ...
    //this.mh = new TextDecoder().decode(mbValeur)
    // this.mh = Buffer.from(mbValeur, 'base64')
...
class VerificateurHachage {
  ...
  verify() {
    ...
    // console.debug("Hachage recu : %O\nHachage Calcule", digestRecu, digestCalcule)

    if( _comparerArraybuffers(digestRecu, digestCalcule) ) {
      return true
    } else {
      throw new Error("Erreur hachage, mismatch")
    }
...
module.exports = {
  hacher, verifierHachage, Hacheur, VerificateurHachage,
}
