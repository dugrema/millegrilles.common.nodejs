...
function socketActionsMiddleware(socket, next) {
  ...
  socket.upgradeProtege = callback => {upgradeProtege(socket, callback)}
  socket.downgradePrive = callback => {downgradePrive(socket, callback)}
  socket.changerApplication = (application, callback) =>{changerApplication(socket, application, callback)}
...
//
//       if(socket.configurationEvenements.millegrilles) {
//         const listenersPrives = socket.configurationEvenementsmillegrilles.listenersPrives
//         debug("Listeners prives millegrilles\n%O", listenersPrives)
//         enregistrerListener(socket, listenersPrives)
//       } else {
//         debug("WARN Listeners millegrilles non initialises - OK pour dev")
...
//   }
//
//   debug("Socket.IO pour %s configure", opts.path)
