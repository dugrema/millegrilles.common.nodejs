...
function changerApplication(socket, application, callback) {
  ...
  if(socket.application) {
    ...
    debug("Listeners prives %s\n%O", application, listenersPrivesApplication)
...
function subscribe(socket, params, callback) {
  ...
  //
  // // Enregistrer les routing keys, garder type.domaine.action  (tronquer * ou #)
  // const domainesActions = getDomainesActions(routingKeys)
  // domainesActions.forEach(domaineAction=>{
  //   debug("Join room %s", domaineAction)
  //   socket.join(domaineAction)
  //   // socket.on(domaineAction, callbackFilter)
  // })
...
function getDomainesActions(routingKeys) {
  debug("Domaines actions, routingKeys : %O", routingKeys)
