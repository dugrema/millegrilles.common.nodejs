...
class MilleGrillesPKI {
      ...
      return new Promise((resolve, reject)=>{
        ...
        fs.readFile(fichier, 'utf8', (err, data)=>{
          ...
          try {
            ...
            const fingerprintCalcule = fingerprintsCalcules.fingerprintSha256B64
            var fingerprintMatch = false
            if(format === 'sha256_b64' && fingerprintCalcule === fingerprintEffectif) {
  ...
  async verifierSignatureMessage(message) {
    ...
    try {
      ...
      debug("Certifcat chaine locale : %O", certificatChaine)
    } catch (err) {
      debug("Erreur chargement chaine localement, verifier d'autres methodes de chargement\n%O", err)
    }
