...
class MilleGrillesPKI {
  ...
  async sauvegarderMessageCertificat(message, fingerprintBase58) {
    ...
    const fichier = path.join(REPERTOIRE_CERTS_TMP, fingerprintBase58 + '.json')
    ...
    if( ! fichierExiste ) {
      ...
      if(store.verifierChaine(chaine_pem, {validityCheckDate: null})) {
        ...
        let fingerprint = hacherCertificathacherCertificat(certificat)
      ...
      debug("Fichier certificat existe deja pour certificat (%s.json)", fingerprintBase58)
    }
  }
