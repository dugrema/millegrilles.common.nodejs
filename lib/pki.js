...
const {hacherCertificat, splitPEMCerts} = require('./formatteurMessage')
...
class MilleGrillesPKI {
  ...
  async chargerCertificats(certPems) {
    ...
    const fingerprint = hacherCertificat(parsedCert)
  ...
  async sauvegarderMessageCertificat(message, fingerprintBase58) {
    ...
    if( ! fichierExiste ) {
      ...
      if(store.verifierChaine(chaine_pem, {validityCheckDate: null})) {
        ...
        let fingerprint = hacherCertificat(certificat)
  ...
  async getCertificate(fingerprint) {
    ...
    if(certificat) {
      ...
      return await new Promise((resolve, reject)=>{
        let fichier = path.join(REPERTOIRE_CERTS_TMP, fingerprintEffectif + '.pem')
