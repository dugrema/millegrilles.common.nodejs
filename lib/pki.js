...
const {hacherCertificat, splitPEMCerts, FormatteurMessage} = require('./formatteurMessage')
...
class MilleGrillesPKI {
  ...
  constructor() {
    ...
    this.cacheCertsParFingerprint = {}  // Cle : fingerprint, value = [...certForge]
  ...
  async verifierMessage(message) {
    ...
    var chaineForge = await this.getCertificate(fingerprint, opts)

    if( ! chaineForge && chaine ) {
      // Charger et valider le certificat inline
      chaineForge = await self.chargerCertificats(chaine)
    }
  ...
  async sauvegarderMessageCertificat(message, fingerprintBase58) {
    ...
    let fichierExiste = fingerprintBase58 && await new Promise((resolve, reject)=>{
      try {
    ...
    if( ! fichierExiste ) {
      ...
      if(store.verifierChaine(chaine_pem, {validityCheckDate: null})) {
        ...
        this.cacheCertsParFingerprint[fingerprint] = chaineCerts
  ...
  async getCertificate(fingerprint, opts) {
    ...
    try {
      certificat = await chargerCertificatFS(fingerprintEffectif)
...
async function chargerCertificatFS(fingerprint) {
