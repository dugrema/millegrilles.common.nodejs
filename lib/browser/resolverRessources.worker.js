...
async function resolveUrlFuuid(fuuid, fuuidInfo) {
  console.debug("resolver.resolveUrlFuuid %s, %O", fuuid, fuuidInfo)
  ...
  if(typeCdn === 'ipfs') {
    ...
    let pathFuuid
    if(prive && estVideo) {
      pathFuuid = path.join('fichiers/video', fuuid + '.' + ext)
    } else if(prive) {
    ...
    let pathFuuid
    if(prive && estVideo) {
      pathFuuid = path.join('/fichiers/video', fuuid)
    } else if(prive) {
    ...
    let pathFuuid
    if(prive && estVideo) {
      pathFuuid = path.join('fichiers/video', part1, part2, fuuid + '.' + ext)
    } else if(prive) {
