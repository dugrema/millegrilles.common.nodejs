...
async function entretienCache(nomUsager, opts) {
  ...
  await Promise.all([
    entretienCacheClesSecretes(nomUsager, opts)
  ])
}

async function entretienCacheClesSecretes(nomUsager, opts) {
  opts = opts || {}
  const nomDB = 'millegrilles.' + nomUsager
  const db = await openDB(nomDB)
  ...
  const tempsExpiration = new Date().getTime() - maxAge,
        dateExpiration = new Date(tempsExpiration)
  console.debug("Entretien table de caches IndexedDB usager (%s), purger elements < %O", nomUsager, dateExpiration)
