...
var clePriveeSubtle = null    // Cle privee format subtle
...
async function initialiserFormatteurMessage(opts) {
  ...
  const clePriveePem = opts.clePriveePem,
        chaineCertificatsPem = opts.chaineCertificatsPem,
        DEBUG = opts.DEBUG

  if(clePriveePem) {
    if(DEBUG) console.debug("Charger cle privee PEM (en parametres)")
    clePriveeSubtle = await importerClePriveeSubtle(clePriveePem)
  } else {
  ...
  if(chaineCertificatsPem) {
    if(DEBUG) console.debug("Utiliser chaine pem fournie : %O", chaineCertificatsPem)
  } else {
  ...
  formatteurMessage = new FormatteurMessageSubtle(chaineCertificatsPem, clePriveeSubtle)
...
async function uploaderFichier(correlation, size, reader, progressCb, opts) {
  ...
  try{
    ...
    const passwordChiffreBuffer = await chiffrerCleSecreteSubtle(clePublique, resultat.password, {DEBUG: true})
    ...
    return {status: reponse.status, meta: resultat.meta, passwordChiffre}
  } catch(err) {
  ...
  if((password || passwordChiffre) && iv && tag) {
    ...
    if(passwordChiffre) {
      ...
      if( clePriveePem ) {
        ...
        if(DEBUG) console.debug("Charger cle privee Subtle a partir de IndexedDB")
        throw new Error("Recuperer cle subtle de IndexedDB - TODO")
      }
    }
