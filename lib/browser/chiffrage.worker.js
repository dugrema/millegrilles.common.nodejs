...
import { FormatteurMessageSubtle, hacherCertificat } from '../formatteurMessage'
...
async function uploaderFichier(correlation, size, reader, progressCb, opts) {
  ...
  try{
    ...
    var commandeMaitrecles = await preparerCommandeMaitrecles(
      certificatPem, resultat.password, 'GrosFichiers', meta.hachage_bytes, meta.iv, meta.tag, {type: 'fichier'})
...
async function _getPublicKeyFromCertificat(certificatPem, opts) {
  ...
  const resultat = {publicKey: publicKeyForge, fingerprint}
  if(DEBUG) console.debug("Resultat public key : %O", resultat)
...
async function dechiffrerDocument(ciphertext, messageCle, opts) {
  opts = opts || {}
  const DEBUG = opts.DEBUG
  ...
  const {iv, tag, cle, format} = messageCle
  ...
  const password = await dechiffrerCleSecreteSubtle(clePriveeSubtleDecrypt, cle, {DEBUG})

  var contenuDocument = null
  if(format === 'mgs2') {
    var documentString = await dechiffrer(ciphertext, password, iv, tag)
    documentString = new TextDecoder().decode(documentString)
