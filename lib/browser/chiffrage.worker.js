...
var callbackCleMillegrille = null  // Callback sur etat de la cle de millegrille
...
async function uploaderFichier(correlation, size, reader, progressCb, opts) {
  ...
  // Valider le certificat pour chiffrage - lance une exception en cas d'erreur
  ...
  const pathUpload = path.join(URL_UPLOAD, correlation)
  try{
    ...
    commandeMaitrecles = await formatterMessage(commandeMaitrecles, 'MaitreDesCles.sauvegarderCle', {attacherCertificat: true})
    ...
    if(transactionGrosFichiers) {
      ...
      transactionGrosFichiers = await formatterMessage(transactionGrosFichiers, 'GrosFichiers.nouvelleVersion', {attacherCertificat: true})
    ...
    return {status: reponse.status, commandeMaitrecles, transactionGrosFichiers}
  } catch(err) {
...
async function _putFichierPartiel(correlation, position, infoUpload, ciphertext, opts) {
  ...
  try {
    const pathUpload = path.join(URL_UPLOAD, correlation, ''+position)
...
async function downloadCacheFichier(url, progressCb, opts) {
  ...
  if(opts.DEBUG) console.debug("Options : %O", opts)
...
async function chargerCleMillegrilleSubtle(clePrivee) {
  ...
  try {
    ...
    // console.debug("Importer cle privee subtle - sign")
    const clePriveeSign = await importerClePriveeSubtle(clePrivee, {
      usage: ['sign'], algorithm: 'RSA-PSS', hash: 'SHA-512'})

    cleMillegrilleSubtle = {
...
async function clearCleMillegrilleSubtle() {
  cleMillegrilleSubtle = null
  try {
    callbackCleMillegrille(false)
  } catch(err) {
    // OK
  }
}
...
async function rechiffrerAvecCleMillegrille(secretsChiffres, pemRechiffrage, opts) {
  ...
  */
  ...
  if(!cleMillegrilleSubtle || !cleMillegrilleSubtle.decrypt) {
  ...
  const promises = Object.keys(secretsChiffres).map(async correlation => {
    ...
    buffer = await dechiffrerCleSecreteSubtle(cleMillegrilleSubtle.decrypt, buffer)
